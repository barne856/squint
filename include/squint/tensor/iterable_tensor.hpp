 ...
 */
template <typename Derived> class iterable_mixin {
  public:
        ...
        requires fixed_tensor<Derived> && fixed_shape<SubviewShape>
    {
        ...
        static_assert(SubviewShape::size() <= Derived::shape_type::size(),
        ...
        auto subview_shape = make_array(typename Derived::shape_type{});
        for (std::size_t i = 0; i < Derived::shape_type::size(); ++i) {
        ...
        auto begin = subview_iterator<Derived, SubviewShape>(
            derived, std::array<std::size_t, Derived::shape_type::size()>{}, subview_shape);
        auto end = subview_iterator<Derived, SubviewShape>(
            derived,
            [end_indices, subview_shape] {
                std::array<std::size_t, Derived::shape_type::size()> result;
        ...
        requires fixed_tensor<Derived> && fixed_shape<SubviewShape>
    {
        ...
        auto begin = subview_iterator<const Derived, SubviewShape>(
            derived, std::array<std::size_t, Derived::shape_type::size()>{}, subview_shape);
        auto end = subview_iterator<const Derived, SubviewShape>(
            derived,
            [end_indices, subview_shape] {
                std::array<std::size_t, Derived::shape_type::size()> result;
                std::transform(end_indices.begin(), end_indices.end(), subview_shape.begin(), result.begin(),
        ...
        requires dynamic_tensor<Derived>
    {
        ...
        if constexpr (derived->error_checking() == error_checking::enabled) {
            ...
            for (std::size_t i = 0; i < subview_shape.size(); ++i) {
                if (derived->shape()[i] % subview_shape[i] != 0) {
        ...
        subview_shape_copy.resize(derived->rank(), 1);

        auto begin = subview_iterator<Derived, std::vector<std::size_t>>(
            derived, std::vector<std::size_t>(derived->rank(), 0), subview_shape_copy);
